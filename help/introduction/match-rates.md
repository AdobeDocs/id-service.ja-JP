...
ページ本文の読み込み中、ID サービスは、*`Destination Publishing iFrame`* と呼ばれる iFrame を読み込みます。 [!UICONTROL ターゲットパブリッシング iFrame] は、親ページとは別のドメインに読み込まれます。この設計によって iFrame は以下の動作をするので、ページパフォーマンスを確保し、セキュリティを強化できます。
