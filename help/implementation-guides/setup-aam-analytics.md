---
description: これらの手順は、AnalyticsおよびAudience Managerのお客様向けに、Visitor ID サービスを使用し、タグを使用しない場合に使用します。 ただし、タグを使用して訪問者ID サービスを実装することを強くお勧めします。 タグは、実装ワークフローを効率化し、適切なコードの配置とシーケンスを自動的に保証します。
keywords: 訪問者 ID サービス
title: AnalyticsおよびAudience Manager用のAdobe Visitor ID サービスの実装
...
# AnalyticsおよびAudience Manager用のAdobe Visitor ID サービスの実装{#implement-the-experience-cloud-id-service-for-analytics-and-audience-manager}

この手順は、AnalyticsおよびAudience Managerのお客様向けに、Visitor ID サービスを使用し、[tags](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=ja)を使用しない場合に表示されます。 ただし、タグを使用して訪問者ID サービスを実装することを強くお勧めします。 タグは、実装ワークフローを効率化し、適切なコードの配置とシーケンスを自動的に保証します。
...
1. **[!UICONTROL Admin]** > **[!UICONTROL Code Manager]**&#x200B;に移動します。
...
## 手順9：訪問者ID サービスコードのテストとデプロイ {#section-f857542bfc70496dbb9f318d6b3ae110}

次のようにテストおよびデプロイできます。

**テストと検証**

訪問者ID サービスの実装をテストするには、次の点を確認します。
...
* [訪問者ID サービスのテストと検証](../implementation-guides/test-verify.md)も参照してください。
